<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Activos Fijos/Depreciacion/Mostrar') }}
        </h2>
    </x-slot>
    <div class="max-w-7xl mx-auto py-10 sm:px-10 lg:px-10 md:grid md:grid-cols-3 md:gap-6 ">
        <x-jet-section-title>
            <x-slot name="title">
                {{ __('Ha escogido mirar la informacion de la depreciacion '). $dpto->nombre }}
            </x-slot>
            <x-slot name="description">
                {{ __('En este formulario tambien es posible editar la información de ') . $dpto->nombre }}
            </x-slot>
        </x-jet-section-title>
        <form action="{{route('depreciacion.update',[$depre->id_depreciacion])}}" method="POST" id="formulario" class="mt-4 md:mt-0 md:col-span-2">
            @csrf
            @method("PUT")
