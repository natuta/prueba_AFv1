<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Activos fijos/Solicitudes de Revalorizacion') }}
        </h2>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
        ...
        <table class="table">
            ...
            <tbody>
            @foreach($revaluos as $rev)
                <tr>
                    <td>{{$rev->id_revaluo}}</td>
                    <td>{{$rev->fecha}}</td>
                    <td>{{$rev->descripcion}}</td>
                    <td>{{$rev->revision->id_revision}}</td>
                    <td>
                        <a href="{{route('revaluos.show',[$rev->id_revaluo])}}" class="inline-flex items-center px-4 py-2 bg-green-400
